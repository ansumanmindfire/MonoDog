...
export function resolveWorkspaceGlobs(
  ...
  for (const glob of globs) {
    if (glob.endsWith('/*')) {
      ...
      if (
        fs.existsSync(baseDirPath) &&
        fs.statSync(baseDirPath).isDirectory()
      ) {
        const subDirs = fs
...
function getWorkspacesFromPnpmYaml(rootDir: string): string[] | undefined {
  ...
  try {
    ...
    AppLogger.error(
      `Error parsing pnpm-workspace.yaml at ${workspaceYamlPath}:`,
      e as Error
    );
  }
  return undefined;
...
async function scanMonorepo(rootDir: string): Promise<PackageInfo[]> {
  ...
  if (detectedWorkspacesGlobs && detectedWorkspacesGlobs.length > 0) {
    ...
    const resolvedPackagePaths = resolveWorkspaceGlobs(
      rootDir,
      detectedWorkspacesGlobs
    );

    AppLogger.debug(
      `Resolved package directories (Total ${resolvedPackagePaths.length})`
    );
...
export async function getRepositoryInfoFromGit(
  ...
  try {
    ...
    if (!owner || !repo) {
      AppLogger.warn(
        `Could not parse repository info from git remote: ${remoteUrl}`
      );
      return null;
    }
